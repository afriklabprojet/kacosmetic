...
function SocialIcon({ label }: { label: string }) {
  if (label.toLowerCase().includes("instagram")) return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      <rect x="2" y="2" width="20" height="20" rx="5" ry="5" /><circle cx="12" cy="12" r="4" /><circle cx="17.5" cy="6.5" r="1" fill="currentColor" stroke="none" />
    </svg>
  )
  if (label.toLowerCase().includes("tiktok")) return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.33 6.33 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V8.69a8.18 8.18 0 0 0 4.78 1.52V6.77a4.85 4.85 0 0 1-1.01-.08z" />
    </svg>
  )
  if (label.toLowerCase().includes("whatsapp")) return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
export default function FooterClient({ categories, socials }: Readonly<{ categories: FooterCategory[]; socials: SocialLink[] }>) {
  ...
  async function handleNewsletter(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) { setStatus("success"); setMessage("Bienvenue dans le Cercle ✨"); setEmail("") }
      else { setStatus("error"); setMessage("Une erreur est survenue. Réessayez.") }
    ...
    <footer className="border-t-2 border-or bg-ivoire pb-8 pt-20 px-4 md:px-8">
      <div className="mx-auto max-w-[1400px]">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-8 mb-16">
          ...
          <div className="lg:col-span-5">
            <Link href="/" aria-label="Ka Cosmetic — Accueil">
              <Image src="/logo.png" alt="Ka Cosmetic" width={110} height={124} className="mb-4" style={{ width: "110px", height: "auto" }} />
            </Link>
            <p className="text-sm text-ebene/70 mb-6 leading-relaxed max-w-xs">Offres privées & rituels exclusifs.</p>
            ...
            {status === "success" ? (
              ...
              <form className="relative group" onSubmit={handleNewsletter} noValidate>
                <input type="email" name="email" value={email} onChange={e => setEmail(e.target.value)}
                  ...
                  required aria-label="Email pour la newsletter" disabled={status === "loading"} />
                ...
                {status === "error" && <p className="mt-2 text-xs text-erreur">{message}</p>}
              </form>
            )}
            ...
            <div className="flex items-center gap-4 mt-6">
              {socials.map((s) => (
                <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
                  aria-label={s.label} className="cursor-pointer text-ebene/50 transition-colors hover:text-or">
          ...
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-3 lg:col-span-7">
            <div>
              <h4 className="mb-5 text-[11px] font-medium uppercase tracking-[0.28em] text-ebene/50">La Maison</h4>
              <ul className="flex flex-col gap-4">
                {FOOTER_LINKS_MAISON.map(l => (
                  <li key={l.href}><Link href={l.href} className="text-sm text-ebene font-light transition-colors hover:text-brun">{l.label}</Link></li>
                ))}
              </ul>
            </div>
            <div>
              <h4 className="mb-5 text-[11px] font-medium uppercase tracking-[0.28em] text-ebene/50">Assistance</h4>
              <ul className="flex flex-col gap-4">
                {FOOTER_LINKS_AIDE.map(l => (
                  <li key={l.href}><Link href={l.href} className="text-sm text-ebene font-light transition-colors hover:text-brun">{l.label}</Link></li>
                ))}
              </ul>
            </div>
            <div>
              <h4 className="mb-5 text-[11px] font-medium uppercase tracking-[0.28em] text-ebene/50">Boutique</h4>
              <ul className="flex flex-col gap-4">
                {categories.map(l => (
                  <li key={l.href}><Link href={l.href} className="text-sm text-ebene font-light transition-colors hover:text-brun">{l.label}</Link></li>
                ))}
        ...
        <div className="mb-10 flex flex-col items-start gap-3 sm:flex-row sm:items-center">
          <span className="text-[10px] uppercase tracking-widest text-ebene/40 shrink-0">Paiements acceptés</span>
          <div className="flex items-center gap-3 flex-wrap">
            {PAYMENT_METHODS.map(p => (
              <div key={p.alt} className="flex h-7 items-center rounded border border-ebene/10 bg-white px-2">
